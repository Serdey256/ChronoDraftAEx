...
var javaPatterns = []struct {
	...
	{regexp.MustCompile(`public\s+(abstract\s+)?class\s+(\w+)`), "class", 2},
	{regexp.MustCompile(`public\s+interface\s+(\w+)`), "interface", 1},
	{regexp.MustCompile(`public\s+enum\s+(\w+)`), "enum", 1},
	{regexp.MustCompile(`import\s+([\w.]+)`), "import", 1},
}

func analyzeJavaFile(filePath string) ([]models.CodeEntity, error) {
