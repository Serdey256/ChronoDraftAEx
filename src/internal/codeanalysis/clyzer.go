...
var cPatterns = []struct {
	pattern    *regexp.Regexp
	entityType string
	nameIdx    int
}{
	{regexp.MustCompile(`struct\s+(\w+)`), "struct", 1},
	{regexp.MustCompile(`class\s+(\w+)`), "class", 1},
	{regexp.MustCompile(`#include\s+[<"]([^>"]+)[>"]`), "import", 1},
}

func analyzeCFile(filePath string) ([]models.CodeEntity, error) {
